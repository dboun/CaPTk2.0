...
class CaPTkItkAdaptiveGeodesicDistanceFilterTestSuite : public mitk::TestFixture
{
    ...
    MITK_TEST(Try2DFloat);
    MITK_TEST(Try2DIntOut);
...
public:
	...
	using FilterType = 
	    itk::captk::AdaptiveGeodesicDistanceFilter< ImageTypeFloat2D, ImageTypeInt2D >;
	FilterType::Pointer filter = FilterType::New();
	...
	WriteImage<ImageTypeInt2D>(filter->GetOutput(), 
			mitk::IOUtil::GetTempPath() + "/CaPTk_out2.nii.gz");
